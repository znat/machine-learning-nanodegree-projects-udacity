    ...
    def step(self):
        ...
        if self.primary_agent is not None and self.done is not True:
            ...
            if agent_deadline <= self.hard_time_limit:
                self.done = True
                self.results.append((0, self.primary_agent.score, self.primary_agent.penalties))
        ...
        if agent is self.primary_agent:
            if state['location'] == state['destination']:
                if state['deadline'] >= 0:
                    reward += 10  # bonus
                self.done = True
                self.results.append((1, self.primary_agent.score, self.primary_agent.penalties))
